...
$getdata = mysqli_query($conn, "SELECT *,case status when '1' then 'Active' else 'Not Active' end as statuslabel 
                                        ,case status when '1' then 'success' else 'danger' end as statuscolor 
                                         from customers where deleted_at is null order by id desc");
                    ...
                    <div class="card-body pb-0">
                                        ...
                                        <?php while ($rows = mysqli_fetch_assoc($getdata)) { ?>
                                            <tr>
                                                ...
                                                <td class="text-center" style="width:20%;">
                                                    ...
                                                    <a href="#" id="delete_<?= $rows['id']; ?>" tid="<?= $rows['id']; ?>" tipe="customers" class="btn btn-danger"><i class="bi bi-trash"></i></a>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    <?php } else { ?>
                        ...
                        <?php } else {
                            if (isset($_GET['tid'])) {
                                ...
                                $getdata = mysqli_query($conn, "SELECT * from customers where id = '$tid'");
                        ...
                        ?>
                            <input type="hidden" id="tid" name="tid" class="customersform" value="<?= $tid; ?>">
                            <h5 class="card-title"><?= $label; ?> Form</h5>
                            <div class="row g-3">
                                <div class="col-md-12 mb-2">
                                    <div class="form-floating">
                                        <input type="text" class="form-control customersform" name="name" id="name" value="<?= $name; ?>" placeholder="Name" required>
                                        <label for="name">Name</label>
                                    </div>
                                </div>
                                <div class="col-md-12 mb-2">
                                    <div class="form-floating">
                                        <input type="text" class="form-control customersform" name="phone" id="phone" value="<?= $phone; ?>" placeholder="Phone" required>
                                        <label for="name">Phone</label>
                                    </div>
                                </div>
                                <div class="col-md-12 mb-2">
                                    <div class="form-floating">
                                        <textarea type="text" class="form-control customersform" name="address" id="address" placeholder="Address"><?= $address; ?></textarea>
